(function () {
  ...
  function renderResults() {
    ...
    const views = { summary: renderSummary, matrix: renderMatrix, tracks: renderTracks, roadmap: renderRoadmap };
  ...
  function exportPayload() {
    const results = engine.evaluate(state.answers, catalog);
    return { generatedAt: new Date().toISOString(), catalogVersion: catalog.version, advisoryOnly: true, answers: Object.fromEntries(catalog.questions.map((question) => [question.label, answerLabel(question, state.answers[question.id])])), recommendations: results.map(({ id, product, track, phase, phaseName, status, reason, action, source, missing }) => ({ id, product, track, phase, phaseName, status, reason, action, missing, source })) };
  }
  ...
  function exportMarkdown() {
    ...
    payload.recommendations.forEach((item) => lines.push(`### Phase ${item.phase}: ${item.product}`, "", `- **Status:** ${statusLabel(item.status)}`, `- **Track:** ${item.track}`, `- **Why:** ${item.reason}`, `- **Next action:** ${item.action}`, `- **Guide:** ${item.source}`, ""));
